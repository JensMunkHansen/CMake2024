...
#ifndef spsCellLocatorT_h
#define spsCellLocatorT_h

#include "vtkAbstractCellLocator.h"
#include "spsCommonDataModelModule.h" // For export macro
...
template <typename T>
class vtkCellLocatorT : public vtkAbstractCellLocator
{
public:
  ///@{
  /**
   ...
   */
  vtkTypeMacro(vtkCellLocatorT, vtkAbstractCellLocator);
   ...
   */
  int IntersectWithLine(const T p1[3], const T p2[3], T tol, T& t, T x[3],
    T pcoords[3], int& subId, vtkIdType& cellId, vtkGenericCell* cell) override;
   ...
   * For other IntersectWithLine signatures, see vtkAbstractCellLocator.
   */
  int IntersectWithLine(const T p1[3], const T p2[3], T tol, vtkPoints* points,
   ...
   */
  void FindClosestPoint(const T x[3], T closestPoint[3], vtkGenericCell* cell,
    vtkIdType& cellId, int& subId, T& dist2) override
  {
   ...
   */
  vtkIdType FindClosestPointWithinRadius(T x[3], T radius, T closestPoint[3],
    vtkGenericCell* cell, vtkIdType& cellId, int& subId, T& dist2, int& inside) override;
   ...
   */
  vtkIdType FindCell(T x[3], T vtkNotUsed(tol2), vtkGenericCell* GenCell, int& subId,
    T pcoords[3], T* weights) override;

  /**
   * Return a list of unique cell ids inside of a given bounding box. The
   * user must provide the vtkIdList to populate.
   */
  void FindCellsWithinBounds(T* bbox, vtkIdList* cells) override;
  ...
  void FindCellsAlongLine(
    const T p1[3], const T p2[3], T tolerance, vtkIdList* cellsIds) override
  {
  ...
  /**
   * Shallow copy of a vtkCellLocatorT.
   *
...
protected:
  ...
  void GetOverlappingBuckets(vtkNeighborCells& buckets, const T x[3], T dist,
    int prevMinLevel[3], int prevMaxLevel[3]);

  inline void GetBucketIndices(const T x[3], int ijk[3]);

  T Distance2ToBucket(const T x[3], int nei[3]);
  T Distance2ToBounds(const T x[3], double bounds[6]);
...
private:
  vtkCellLocatorT(const vtkCellLocatorT&) = delete;
  void operator=(const vtkCellLocatorT&) = delete;
};
