...
function(sps_scan_wrappers folder output_var)
    # Glob all .cxx files in the specified folder
    file(GLOB WRAPPER_FILES "${folder}/*.cxx")

    # Create an empty list to store stripped file names
    set(STRIPPED_FILES)

    # Loop through each file and strip the extension
    foreach(FILE_PATH ${WRAPPER_FILES})
        get_filename_component(FILE_NAME ${FILE_PATH} NAME_WE)
        list(APPEND STRIPPED_FILES ${FILE_NAME})
    endforeach()

    # Return the stripped file names in the specified output variable
    set(${output_var} ${STRIPPED_FILES} PARENT_SCOPE)
endfunction()
...
include("${_vtkModule_dir}/vtkTopologicalSort.cmake")
...
function (_vtk_module_write_import_prefix file destination)
  ...
  file(WRITE "${file}"
    "set(_vtk_module_import_prefix \"\${_vtkModule_dir}\")\n")
  while (destination)
...
include("${_vtkModule_dir}/vtkModuleTesting.cmake")
...
include("${_vtkModule_dir}/vtkModuleSerialization.cmake")

#[==[.rst:
.. cmake:command:: sps_module_add_module

  Create a module library. |module|

  .. code-block:: cmake

     sps_module_add_module(<name>
...
#]==]
function (sps_module_add_module name)
  ...
  foreach (_vtk_add_module_kind IN ITEMS CLASSES TEMPLATE_CLASSES HEADERS TEMPLATES VTK_JAVASCRIPT_WRAPPERS SPS_JAVASCRIPT_WRAPPERS)
  ...
  foreach (_vtk_add_module_class IN LISTS _vtk_add_module_VTK_JAVASCRIPT_WRAPPERS)
    message("${_vtk_add_module_class}")
  endforeach()
