...
def interactionFcn(caller, event):
  global clipperPoly
  boxWidget = caller
...
def saveFcn(caller, event):
  global clipperPoly
  if caller.GetKeySym() == 'o':
    writer = vtkPLYWriter()
    writer.SetFileName("out.ply")
    writer.SetInputConnection(clipperPoly.GetOutputPort())
    writer.Write()
def main(argv):
  global clipperPoly
  # Box widget
  ...
  windowInteractor.AddObserver(vtkCommand.KeyPressEvent, saveFcn)
  ...
  boxWidget.AddObserver(vtkCommand.InteractionEvent, interactionFcn)
