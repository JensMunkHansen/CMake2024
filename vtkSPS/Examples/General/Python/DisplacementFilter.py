    ...
    def GetPolyDataDistance(self, mesh, src):
        ...
        for ptId in range(numPts):
            if (self.Gradient):
                ...
                gradient = [0,0,0]
                dist = 0.0
                # 1. Use vtkImplicitPolyDataDistance to compute the distance and gradient
                ...
                pt = mesh.GetPoint(ptId)
                closestPoint = [0,0,0]
                # 2. Use vtkImplicitPolyDataDistance to compute the distance and direction (normalized gradient)
        ...
        # Calculate distance from cell centers (not the points).
        if (self.ComputeCellCenterDisplacement):
            ...
            for cellId in range(numCells):
              # 3. Compute direciton and distance for cell centers (tricky)
