...
REM Ensure GTEST is build with CMAKE_DEBUG_POSTFIX=d and BUILD_SHARED_LIBS=ON
REM TODO: Re-use content from many bat files (or use PowerShell)
REM TODO: Ensure DLLs not deployed to install dir.

REM TODO: Use file(GET_RUNTIME_DEPENDENCIES) and add it to install. Store it in CMake/get_runtime_deps.cmake
...
cmake -H%~dp0/LibraryUsage -B%~dp0/LibraryUsage/build -G "Visual Studio 17 2022" -A "x64" -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Release
...
cmake --install . --prefix=%~dp0/install
goto :eof

:Debug

cmake -H%~dp0/Library -B%~dp0/Library/build -G "Visual Studio 17 2022" -A "x64" -DBUILD_SHARED_LIBS=ON -DCMAKE_PREFIX_PATH="%GTEST_CMAKE_DIR%"
cmake -H%~dp0/LibraryUsage -B%~dp0/LibraryUsage/build -G "Visual Studio 17 2022" -A "x64" -DBUILD_SHARED_LIBS=ON -DCMAKE_BUILD_TYPE=Debug
